...
print("Введите массив(каждое число через пробел):")
s = input().split() 
A = [int(i) for i in s] 
...
for i in range(len(A)):
    if A[i]==X:
        kol+=1
print(f"Число {X} встречалось в массиве {kol} - раз")
...
s = input().split() 
A = [int(i) for i in s] 
X = int(input("Введите близкое число: "))
raz=abs(X-A[0])
k=0
for i in range(1,len(A)):
    if abs(A[i]-X)<raz:
        raz=abs(A[i]-X)
        k=i
print(f"Самый близкий по величине элемент A[{k+1}] = {A[k]} к заданному {X}")
...
print(f"Стоимость: {word} = {score} очков")
